...
To delete a value $v$ after search in a 2-3 tree, we follow these steps:
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                \item \textbf{Parent Case:} To balance the tree requires taking from a parent key. If OP's parent is the root, 
                we promote OP as the new root.
        ...
        repeatedly until it's a leaf node. Then follow the above steps to delete $v$.
    \end{enumerate}
...
\begin{figure}[ht!] 
    ...
    \end{center}
     \caption{Definition (\ref{def:2-3_tree_delete})'s bullets -- 2K Parent (a \& b).}
