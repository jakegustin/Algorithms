...
\begin{Def}[Simplified NFET \& PFET Symbols]
  ...
  \noindent
  The following simplifies the NFET and PFET symbols from Figure (\ref{fig:mosfet-symbol}) for 
  repetitive use:
  ...
  \noindent
  (1) NFET, conducts when $A$ is high; (2) PFET, conducts when $A$ is low (i.e., $\overline{A}$).

\end{Def}

\newpage

\begin{Def}[MOSFET Logic Gates -- AND, NAND]

  \label{def:mosfet_logic_gates}

  Below shows logic gates, AND and NAND, using MOSFETs (NFET and PFET):

  \begin{center}
  \includegraphics[width=\textwidth]{Sections/circuits/and_nand.png}
  \end{center}

  \noindent
  (1) AND gate, conducts when both $A$ and $B$ are high ($A\cdot B$); (2) NAND gate, conducts when $A$ or $B$ is low ($\overline{A}+\overline{B}=\overline{A\cdot B}$).
\end{Def}
